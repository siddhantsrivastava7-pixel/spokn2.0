...
use crate::formatting::{self, detect_app_kind, FormattingConfig, FormattingContext};
...
pub(crate) async fn process_transcription_output(
    ...
    // Smart Formatting — deterministic, fully-local shaping pass.
    // Runs last so it sees the already-converted / LLM-processed text, and
    // is gated behind a user-toggleable setting so it's a no-op if disabled.
    if settings.smart_formatting_enabled {
        ...
        let formatted = formatting::format(&final_text, &cfg, &ctx);
        if formatted != final_text {
            if post_processed_text.is_none() {
                post_processed_text = Some(formatted.clone());
            }
