...
/// "started". v0.3.7: dropped to 30ms (one frame) — the
/// AudioRecordingManager is now in AlwaysOn mode while Conversation
/// Mode is active, and the recorder's internal VAD prefill captures
/// pre-onset audio. We can be aggressive here without losing
/// reliability because the recorder's smoother filters real noise.
const SPEECH_ONSET_MS: u64 = 30;
